...
void RenderQueueToImage(Image& dst, const FontResource& font, const bk::RenderQueue& queue)
{
    for (const bk::RenderCommand& command : queue.Commands())
    {
        ...
        {
            FillRect(dst, x, y, width, height, r, g, b, a);
            continue;
        }

        if (command.type == bk::RenderCommandType::Text)
        {
            DrawText(dst, font, ToUtf32(command.text), x, y, command.fontSize, r, g, b);
        }
    }
}

Image BuildUiPreviewPanel(const FontResource& font, const bk::InputState& input)
{
    Image panel = MakeImage(532, 228, 255, 255, 255, 238);
    FillRect(panel, 0, 0, panel.width, 2, 88, 101, 242, 255);
    FillRect(panel, 0, panel.height - 1, panel.width, 1, 221, 226, 234, 255);

    bk::RenderQueue queue;

    auto root = std::make_shared<bk::VBox>();
    root->SetSpacing(10.0F);
    root->SetFrame(bk::Rect{20.0F, 18.0F, 492.0F, 192.0F});

    auto title = std::make_shared<bk::Label>("New UI layer preview");
    title->SetFontSize(26.0F);
    title->SetTextColor(bk::Color{0.09F, 0.14F, 0.20F, 1.0F});
    root->AddChild(title);

    auto subtitle = std::make_shared<bk::Label>("Renderer / View / Box / Label / Button");
    subtitle->SetFontSize(18.0F);
    subtitle->SetTextColor(bk::Color{0.38F, 0.43F, 0.50F, 1.0F});
    root->AddChild(subtitle);

    auto pointer = std::make_shared<bk::Label>(
        "Pointer: (" + FormatFloat(input.mousePosition.x) + ", " + FormatFloat(input.mousePosition.y) + ")");
    pointer->SetFontSize(20.0F);
    pointer->SetTextColor(bk::Color{0.18F, 0.22F, 0.27F, 1.0F});
    root->AddChild(pointer);

    auto row = std::make_shared<bk::HBox>();
    row->SetSpacing(12.0F);

    auto primaryButton = std::make_shared<bk::Button>("Common font API");
    primaryButton->SetBackgroundColor(bk::Color{0.18F, 0.45F, 0.90F, 1.0F});
    row->AddChild(primaryButton);

    auto secondaryButton = std::make_shared<bk::Button>(input.mouseLeftDown || input.touchCount > 0 ? "Pointer active" : "Pointer idle");
    secondaryButton->SetBackgroundColor(
        input.mouseLeftDown || input.touchCount > 0
            ? bk::Color{0.00F, 0.59F, 0.53F, 1.0F}
            : bk::Color{0.40F, 0.45F, 0.52F, 1.0F});
    row->AddChild(secondaryButton);

    root->AddChild(row);
    root->Layout();
    root->Render(queue);
    RenderQueueToImage(panel, font, queue);
    return panel;
}
...
int main(int argc, char** argv)
{
    try
    {
        ...
        const int uiPanelX = 656;
        const int uiPanelY = 164;
        const int uiPanelWidth = 532;
        const int uiPanelHeight = 180;
        const int dragLayerX = 700;
        const int dragLayerY = 372;
        ...
        const Image staticBackground = [&]() {
            ...
            DrawText(bg, font, U"简体中文: 字体链与图像资源展示", 656, 200, 24.0F, 31, 41, 55);
            DrawText(bg, font, U"繁體中文: 系統字型與回退測試", 656, 236, 24.0F, 31, 41, 55);
            DrawText(bg, font, U"日本語: システムフォント対応", 656, 272, 24.0F, 31, 41, 55);
            DrawText(bg, font, U"한국어: 폰트 폴백 테스트", 656, 308, 24.0F, 31, 41, 55);

            DrawText(bg, font, U"Material Icons", 656, 336, 22.0F, 24, 35, 52);
            DrawText(bg, font, U"\uE88A  \uE87C  \uE8B8  \uE87D", 656, 364, 18.0F, 37, 99, 235);
            DrawText(bg, font, U"\uE88A  \uE87C  \uE8B8  \uE87D", 656, 396, 28.0F, 0, 150, 136);
        ...
        const std::array<Vertex, 6> statusVertices = MakePixelQuad(
            ...
            screenWidth,
            screenHeight);
        const std::array<Vertex, 6> uiVertices = MakePixelQuad(
            uiPanelX,
            uiPanelY,
            uiPanelWidth,
            uiPanelHeight,
        ...
        Image uiLayer = BuildUiPreviewPanel(font, initialInput);
        int dragSquareX = 72;
        int dragSquareY = 34;
        ...
        bk::BufferHandle uiVertexBuffer = device->CreateBuffer(bk::BufferDesc{
            bk::BufferUsage::Vertex,
            sizeof(Vertex) * uiVertices.size(),
            uiVertices.data(),
        });
        bk::TextureHandle uiTexture = device->CreateTexture(bk::TextureDesc{
            static_cast<std::uint32_t>(uiLayer.width),
            static_cast<std::uint32_t>(uiLayer.height),
            uiLayer.pixels.data(),
        });
        ...
        if (!bk::IsValid(backgroundVertexBuffer) || !bk::IsValid(svgVertexBuffer) || !bk::IsValid(statusVertexBuffer) || !bk::IsValid(uiVertexBuffer) || !bk::IsValid(dragVertexBuffer) ||
            !bk::IsValid(backgroundTexture) || !bk::IsValid(svgTexture) || !bk::IsValid(statusTexture) || !bk::IsValid(uiTexture) || !bk::IsValid(dragTexture) ||
        ...
        while (platform->IsRunning())
        {
            ...
            uiLayer = BuildUiPreviewPanel(font, input);
            ...
            if (!device->UpdateTexture(uiTexture, bk::TextureDesc{
                static_cast<std::uint32_t>(uiLayer.width),
                static_cast<std::uint32_t>(uiLayer.height),
                uiLayer.pixels.data(),
            }))
            {
                std::fprintf(stderr, "Failed to update UI texture.\n");
                break;
            }
            ...
            device->BindVertexBuffer(commandBuffer, uiVertexBuffer);
            device->BindTexture(commandBuffer, uiTexture);
            device->Draw(commandBuffer, static_cast<std::uint32_t>(uiVertices.size()));
        ...
        device->DestroyTexture(uiTexture);
        ...
        device->DestroyBuffer(uiVertexBuffer);
